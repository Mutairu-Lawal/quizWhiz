...
export function checkTime(getUser) {
    ...
    if (hrs !== '00') {
        getUser.time = `${hrs} ${pluralize(hours, 'hour')} ${mins} ${pluralize(minutes, 'minute')} ${secs} ${pluralize(seconds, 'second')}`;
    }
    else if (mins !== '00') {
        getUser.time = `${mins} ${pluralize(minutes, 'minute')} ${secs} ${pluralize(seconds, 'second')}`;
    }
    else {
...
export function returnStringValue(value) {
